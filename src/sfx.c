...
void init_sound() {
    ...
    ASSERT(Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 2048) == 0,
           "SDL mixer faild to initialize %s\n",
           Mix_GetError());

    char* exe_path = SDL_GetBasePath();
    char sounds_path[1024];
    u32 written;
    written = snprintf(sounds_path, sizeof(sounds_path), "%s/../assets/sounds/", exe_path);
    char sound_path_buff[1024];
    written = snprintf(sound_path_buff,
                       sizeof(sound_path_buff),
                       "%s%s",
                       sounds_path,
                       SHOOT_SOUND_NAME);
    if (written >= sizeof(sounds_path)) {
        printf("Error: Path too long or encoding error\n");
    } else {

        _sounds[SFX_SHOOT] = Mix_LoadWAV(sound_path_buff);
        if (_sounds[SFX_SHOOT] == NULL) {
    ...
    if (written >= sizeof(sounds_path)) {
        ...
        if (_sounds[SFX_ROCK_MEDIUM] == NULL) {
            printf("error loading %s\n", BANG_MEDIUM_SOUND_NAME);
        }
    }
    written = snprintf(sound_path_buff,
    ...
    if (written >= sizeof(sounds_path)) {
        ...
        if (_sounds[SFX_ROCK_SMALL] == NULL) {
            printf("error loading %s\n", BANG_SMALL_SOUND_NAME);
        }
    }
    ...
    if (written >= sizeof(sounds_path)) {
        ...
        if (_sounds[SFX_EXTRA_LIFE] == NULL) {
            printf("error loading %s\n", EXTRA_LIFE_NAME);
        }
    }
}
