...
void rock_render(EntityRock* rock) {
    ...
    for (i32 i = 0; i < rock->num_vertices; i++) {
        ...
        V2f32 rotated_current = rotate_point(rock->vertices[i],
                                             (V2f32) { 0, 0 },
                                             deg_to_rad(rock->angle_deg));
        V2f32 rotated_next = rotate_point(rock->vertices[next],
                                          (V2f32) { 0, 0 },
                                          deg_to_rad(rock->angle_deg));

        gfx_render_thick_line(
                state.renderer,
                (i32)rotated_current.x + rock->common.position.x,
                (i32)rotated_current.y + rock->common.position.y,
                (i32)rotated_next.x + rock->common.position.x,
                (i32)rotated_next.y + rock->common.position.y,
                LINE_THICKNESS,
    ...
    if (rock->phantom.phantom_enabled) {
        for (i32 i = 0; i < rock->num_vertices; i++) {
            i32 next = (i + 1) % rock->num_vertices;
            V2f32 rotatedCurrent = rotate_point(rock->vertices[i],
                                                (V2f32) { 0, 0 },
                                                deg_to_rad(rock->angle_deg));
            V2f32 rotatedNext = rotate_point(rock->vertices[next],
                                             (V2f32) { 0, 0 },
                                             deg_to_rad(rock->angle_deg));
            ...
            gfx_render_thick_line(
                    ...
                    (i32)rotatedNext.y + rock->phantom.position.y,
                    LINE_THICKNESS,
                    (SDL_Color) { .r = 255, .g = 255, .b = 255, .a = 255 });
        }
    }
...
bool entity_check_collision_point(Entity* entity, V2f32 point) {
    ...
    V2f32 phantom_point;
    if (rock->phantom.phantom_enabled) {
        phantom_point = (V2f32) { .x = point.x - rock->phantom.position.x,
                                  .y = point.y - rock->phantom.position.y };
        phantom_point = rotate_point(phantom_point,
                                     (V2f32) { 0 },
                                     deg_to_rad(-rock->angle_deg));
    }

    point = (V2f32) { .x = point.x - rock->common.position.x,
                      .y = point.y - rock->common.position.y };
    point = rotate_point(point, (V2f32) { 0 }, deg_to_rad(-rock->angle_deg));

    for (i32 i = 0, j = rock->num_vertices - 1; i < rock->num_vertices;
         j = i++) {
        if (((rock->vertices[i].y > point.y) !=
             (rock->vertices[j].y > point.y)) &&
            (point.x <
             (rock->vertices[j].x - rock->vertices[i].x) *
                             (point.y - rock->vertices[i].y) /
                             (rock->vertices[j].y - rock->vertices[i].y) +
                     rock->vertices[i].x))
            inside = !inside;
    }

    if (inside || !rock->phantom.phantom_enabled) {
        return inside;
    }
    ...
    for (i32 i = 0, j = rock->num_vertices - 1; i < rock->num_vertices;
         j = i++) {
        if (((rock->vertices[i].y > phantom_point.y) !=
             (rock->vertices[j].y > phantom_point.y)) &&
            (phantom_point.x <
             (rock->vertices[j].x - rock->vertices[i].x) *
                             (phantom_point.y - rock->vertices[i].y) /
                             (rock->vertices[j].y - rock->vertices[i].y) +
                     rock->vertices[i].x))
            inside = !inside;
    }
